...
def ftd1(x): return x[:, 0:6, :, :, :]


def ftd2(x): return x[:, 6:12, :, :, :]


def ftd3(x): return x[:, 12:18, :, :, :]


def ftd4(x): return x[:, 18:24, :, :, :]
...
def MidyNet(c_conf=(6, 2, 14, 30), p_conf=(6, 2, 14, 30), t_conf=(4, 2, 14, 30), external_dim=17, nb_residual_unit=4):
    ...
    len_trend = t_conf[0] if t_conf else 0

    # main input
    main_inputs = []
    outputs = []
    if len_period > 0:
        print('171 尺寸', len_closeness, len_period, len_trend, external_dim, nb_residual_unit, map_height, map_width)
